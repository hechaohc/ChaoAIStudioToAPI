...
class BrowserManager {
    constructor(logger, config, authSource) {
        ...
        if (this.config.lowMemoryMode) {
            Object.assign(this.firefoxUserPrefs, {
    ...
    async _waitForWebSocketInit(
        ...
        try {
            while (Date.now() - startTime < timeout) {
                ...
                if (Math.random() < 0.3) {
                    try {
                        const vp = page.viewportSize() || this._getViewportSize();
    ...
    _getViewportSize() {
        if (this.config.lowMemoryMode) {
            return {
                height: this.config.lowMemoryViewportHeight,
                width: this.config.lowMemoryViewportWidth,
            };
        }
    ...
    async _setupLowMemoryResourceRouting(context, authIndex) {
        if (!this.config.lowMemoryMode) return;

        const blockedResourceTypes = new Set(this.config.lowMemoryBlockedResourceTypes || []);
        ...
        this.logger.debug(`[Context#${authIndex}] Low memory resource routing enabled.`);
    }
    ...
    async _saveDebugArtifacts(suffix = "final", authIndex = null, explicitPage = null) {
        if (!this.config.enableDebugArtifacts) {
            this.logger.debug("[Debug] Debug artifact capture is disabled.");
            return;
        }
        ...
        try {
            ...
            await this._cleanupDebugArtifacts();
        } catch (e) {
            this.logger.error(`[Debug] Failed to save debug artifacts: ${e.message}`);
        }
    }

    async _cleanupDebugArtifacts() {
        const retention = this.config.debugArtifactRetention;
        ...
        const debugFiles = files
            .filter(file => /^debug_(screenshot|page_source)_.*\.(png|html)$/.test(file))
            .map(file => {
        ...
        const filesToDelete = debugFiles.slice(retention * 2);
        for (const item of filesToDelete) {
            await fs.promises.unlink(item.filePath).catch(error => {
                this.logger.debug(`[Debug] Failed to remove old artifact ${item.filePath}: ${error.message}`);
            });
        }
    }
    ...
    async _startBackgroundWakeup() {
        ...
        while (this.page && !this.page.isClosed()) {
            try {
                ...
                const vp = currentPage.viewportSize() || this._getViewportSize();
    ...
    async _initializeContext(authIndex, isBackgroundTask = false) {
        ...
        try {
            ...
            const randomWidth = viewport.width + (this.config.lowMemoryMode ? 0 : Math.floor(Math.random() * 50));
            const randomHeight = viewport.height + (this.config.lowMemoryMode ? 0 : Math.floor(Math.random() * 50));
            ...
            await this._setupLowMemoryResourceRouting(context, authIndex);
