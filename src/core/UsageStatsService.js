...
class UsageStatsService {
    constructor(authSource, logger, dataDir, enabled = true, options = {}) {
        ...
        this.maxFileSizeBytes = this._normalizeLimit(options.maxFileSizeBytes, 10 * 1024 * 1024);
        this.maxRecords = this._normalizeLimit(options.maxRecords, 1000);
    ...
    finishRequest(requestId, result = {}) {
        ...
        const shouldRewriteForRecordLimit = this._enforceRecordLimit();
        ...
        this._appendRecord(record, shouldRewriteForRecordLimit);
    ...
    _appendRecord(record, forceRewrite = false) {
        if (!this.enabled) return;
        ...
        this.appendPromise = this.appendPromise
            ...
            .then(async () => {
                ...
                await fs.promises.appendFile(this.statsFilePath, line, "utf-8");
                if (this.maxFileSizeBytes > 0) {
                    const stat = await fs.promises.stat(this.statsFilePath).catch(() => null);
    ...
    async _readRecordsFromFileAsync() {
        try {
            const content = await this._readBoundedStatsFileAsync();
    ...
    _readRecordsFromFile() {
        try {
            const content = this._readBoundedStatsFileSync();
            return { records: this._parseRecordsContent(content) };
        } catch (error) {
            if (error?.code === "ENOENT") {
                return { records: [] };
    ...
    async _rewriteRecordsFile() {
        ...
        const fileContent = this.records.map(record => JSON.stringify(record)).join("\n");
        await fs.promises.writeFile(this.statsFilePath, fileContent ? `${fileContent}\n` : "", "utf-8");
    }

    _readBoundedStatsFileSync() {
        if (this.maxFileSizeBytes < 1 || !fs.existsSync(this.statsFilePath)) {
            return fs.readFileSync(this.statsFilePath, "utf-8");
        }

        const stat = fs.statSync(this.statsFilePath);
        if (stat.size <= this.maxFileSizeBytes) {
            return fs.readFileSync(this.statsFilePath, "utf-8");
        }

        const fd = fs.openSync(this.statsFilePath, "r");
        try {
            const buffer = Buffer.alloc(this.maxFileSizeBytes);
            fs.readSync(fd, buffer, 0, this.maxFileSizeBytes, stat.size - this.maxFileSizeBytes);
            return this._dropPartialFirstLine(buffer.toString("utf-8"));
        } finally {
            fs.closeSync(fd);
        }
    }

    async _readBoundedStatsFileAsync() {
        if (this.maxFileSizeBytes < 1) {
            return fs.promises.readFile(this.statsFilePath, "utf-8");
        }

        const stat = await fs.promises.stat(this.statsFilePath);
        if (stat.size <= this.maxFileSizeBytes) {
            return fs.promises.readFile(this.statsFilePath, "utf-8");
        }

        const handle = await fs.promises.open(this.statsFilePath, "r");
        try {
            const buffer = Buffer.alloc(this.maxFileSizeBytes);
            await handle.read(buffer, 0, this.maxFileSizeBytes, stat.size - this.maxFileSizeBytes);
            return this._dropPartialFirstLine(buffer.toString("utf-8"));
        } finally {
            await handle.close();
        }
