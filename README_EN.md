...
| `LOW_MEMORY_MODE`               | Low memory mode. Reduces browser viewport size and limits Firefox content processes; useful for 4-core/4GB multi-account deployments.                                                                                                                                   | `false`   |
| `LOW_MEMORY_BLOCK_RESOURCE_TYPES`| Resource types to block in low memory mode, comma-separated. Leave empty for best compatibility. Supported values: `image,font,media,stylesheet`.                                                                                                                       | Empty     |
| `LOW_MEMORY_VIEWPORT_WIDTH`     | Browser viewport width in low memory mode.                                                                                                                                                                                                                           | `1280`    |
| `LOW_MEMORY_VIEWPORT_HEIGHT`    | Browser viewport height in low memory mode.                                                                                                                                                                                                                          | `720`     |
| `MAX_REQUEST_BODY_SIZE`         | Maximum request body size buffered by the server. Supports `KB`/`MB`/`GB` suffixes; set to `0` for unlimited.                                                                                                                                                        | `50MB`    |
...
| `USAGE_STATS_MAX_RECORDS`   | Maximum recent usage records retained in memory and `data/usage-stats.jsonl`; set to `0` to keep only current-process aggregate counters.                                             | `1000`        |
| `USAGE_STATS_MAX_FILE_SIZE` | Maximum stats file bytes read at startup. Larger files load only the newest tail and are pruned on later writes.                                                                      | `10MB`        |
| `ENABLE_DEBUG_ARTIFACTS`    | Save browser failure screenshots and HTML files. Disabled by default to avoid disk growth.                                                                                            | `false`       |
| `DEBUG_ARTIFACT_RETENTION`  | Number of screenshot/page-source pairs to retain when debug artifacts are enabled.                                                                                                    | `5`           |
