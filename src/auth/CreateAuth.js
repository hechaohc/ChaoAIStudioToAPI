...
class CreateAuth {
    ...
    async startVncSession(req, res) {
        ...
        try {
            ...
            const maxWidth = this.config.vncMaxWidth || 1280;
            const maxHeight = this.config.vncMaxHeight || 720;
            const screenWidth = Math.min(requestedWidth, maxWidth);
            const screenHeight = Math.min(requestedHeight, maxHeight);

            const screenResolution = `${screenWidth}x${screenHeight}x24`;
            this.logger.info(
                `[VNC] Requested VNC resolution: ${requestedWidth}x${requestedHeight}; using ${screenWidth}x${screenHeight}`
            );
