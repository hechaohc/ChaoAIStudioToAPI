...
class ConfigLoader {
    ...
    loadConfiguration() {
        const config = {
            ...
            checkUpdate: true,
            debugArtifactRetention: 5,
            enableAuthUpdate: true,
            enableDebugArtifacts: false,
            ...
            lowMemoryBlockedResourceTypes: [],
            lowMemoryMode: false,
            lowMemoryViewportHeight: 720,
            lowMemoryViewportWidth: 1280,
            maxContexts: 1,
            maxRequestBodySizeBytes: 50 * 1024 * 1024,
            maxRetries: 3,
            retryDelay: 2000,
            ...
            switchOnUses: 40,
            usageStatsMaxFileSizeBytes: 10 * 1024 * 1024,
            usageStatsMaxRecords: 1000,
            vncMaxHeight: 720,
            vncMaxWidth: 1280,
            wsPort: 9998,
        };
        ...
        if (process.env.LOW_MEMORY_MODE) config.lowMemoryMode = process.env.LOW_MEMORY_MODE.toLowerCase() === "true";
        if (process.env.LOW_MEMORY_BLOCK_RESOURCE_TYPES) {
            const allowedTypes = new Set(["font", "image", "media", "stylesheet"]);
            config.lowMemoryBlockedResourceTypes = process.env.LOW_MEMORY_BLOCK_RESOURCE_TYPES.split(",")
                .map(type => type.trim().toLowerCase())
                .filter(type => allowedTypes.has(type));
        }
        if (process.env.LOW_MEMORY_VIEWPORT_WIDTH) {
            const parsed = parseInt(process.env.LOW_MEMORY_VIEWPORT_WIDTH, 10);
            config.lowMemoryViewportWidth = Number.isFinite(parsed)
                ? Math.max(800, parsed)
                : config.lowMemoryViewportWidth;
        }
        if (process.env.LOW_MEMORY_VIEWPORT_HEIGHT) {
            const parsed = parseInt(process.env.LOW_MEMORY_VIEWPORT_HEIGHT, 10);
            config.lowMemoryViewportHeight = Number.isFinite(parsed)
                ? Math.max(600, parsed)
                : config.lowMemoryViewportHeight;
        }
        if (process.env.MAX_REQUEST_BODY_SIZE) {
            config.maxRequestBodySizeBytes = this._parseSizeToBytes(
                process.env.MAX_REQUEST_BODY_SIZE,
                config.maxRequestBodySizeBytes
            );
        }
        if (process.env.USAGE_STATS_MAX_RECORDS) {
            const parsed = parseInt(process.env.USAGE_STATS_MAX_RECORDS, 10);
            config.usageStatsMaxRecords = Number.isFinite(parsed)
                ? Math.max(0, parsed)
                : config.usageStatsMaxRecords;
        }
        if (process.env.USAGE_STATS_MAX_FILE_SIZE) {
            config.usageStatsMaxFileSizeBytes = this._parseSizeToBytes(
                process.env.USAGE_STATS_MAX_FILE_SIZE,
                config.usageStatsMaxFileSizeBytes
            );
        }
        if (process.env.VNC_MAX_WIDTH) {
            const parsed = parseInt(process.env.VNC_MAX_WIDTH, 10);
            config.vncMaxWidth = Number.isFinite(parsed) ? Math.max(640, parsed) : config.vncMaxWidth;
        }
        if (process.env.VNC_MAX_HEIGHT) {
            const parsed = parseInt(process.env.VNC_MAX_HEIGHT, 10);
            config.vncMaxHeight = Number.isFinite(parsed) ? Math.max(480, parsed) : config.vncMaxHeight;
        }
        ...
        if (process.env.ENABLE_DEBUG_ARTIFACTS)
            config.enableDebugArtifacts = process.env.ENABLE_DEBUG_ARTIFACTS.toLowerCase() === "true";
        if (process.env.DEBUG_ARTIFACT_RETENTION) {
            const parsed = parseInt(process.env.DEBUG_ARTIFACT_RETENTION, 10);
            config.debugArtifactRetention = Number.isFinite(parsed)
                ? Math.max(0, parsed)
                : config.debugArtifactRetention;
        }
    ...
    _parseSizeToBytes(rawValue, fallback) {
        const value = String(rawValue || "")
            .trim()
            .toLowerCase();
        const match = value.match(/^(\d+(?:\.\d+)?)(b|kb|k|mb|m|gb|g)?$/);
        if (!match) {
            this.logger.warn(`[Config] Invalid size value "${rawValue}", using ${fallback} bytes.`);
            return fallback;
        }
    ...
    _printConfiguration(config) {
        ...
        this.logger.info(
            `  Debug Artifacts: ${config.enableDebugArtifacts ? `Enabled (keep ${config.debugArtifactRetention})` : "Disabled"}`
        );
        ...
        this.logger.info(
            `  Usage Stats Retention: ${config.usageStatsMaxRecords || "No request history"} records, max file ${config.usageStatsMaxFileSizeBytes} bytes`
        );
        this.logger.info(
            `  Low Memory Mode: ${
                config.lowMemoryMode
                    ? `Enabled (${config.lowMemoryViewportWidth}x${config.lowMemoryViewportHeight}, block=${
                          config.lowMemoryBlockedResourceTypes.join(",") || "none"
                      })`
                    : "Disabled"
            }`
        );
        this.logger.info(`  VNC Max Resolution: ${config.vncMaxWidth}x${config.vncMaxHeight}`);
        this.logger.info(`  Max Request Body Size: ${config.maxRequestBodySizeBytes} bytes`);
