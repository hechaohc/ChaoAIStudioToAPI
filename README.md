...
| `LOW_MEMORY_MODE`               | 低内存模式。会缩小浏览器视口并限制 Firefox 内容进程；适合 4H4G 同时跑多账号。                                                                                        | `false`   |
| `LOW_MEMORY_BLOCK_RESOURCE_TYPES`| 低内存模式下要拦截的资源类型，逗号分隔。为空时保持最高兼容性；可选值：`image,font,media,stylesheet`。                                                                  | 空        |
| `LOW_MEMORY_VIEWPORT_WIDTH`     | 低内存模式下的浏览器视口宽度。                                                                                                                                      | `1280`    |
| `LOW_MEMORY_VIEWPORT_HEIGHT`    | 低内存模式下的浏览器视口高度。                                                                                                                                      | `720`     |
| `MAX_REQUEST_BODY_SIZE`         | 服务端允许缓冲的最大请求体大小，支持 `KB`/`MB`/`GB` 后缀；设为 `0` 表示不限制。                                                                                     | `50MB`    |
...
| `USAGE_STATS_MAX_RECORDS`   | 请求统计最多保留的最近记录数，用于控制内存和 `data/usage-stats.jsonl` 体积；设为 `0` 仅保留当前进程聚合计数。 | `1000`   |
| `USAGE_STATS_MAX_FILE_SIZE` | 启动时最多读取的统计文件大小，超出后只读取最新部分并在后续写入时修剪文件。                               | `10MB`   |
| `ENABLE_DEBUG_ARTIFACTS`    | 是否在浏览器失败时保存截图和 HTML；默认关闭以避免磁盘增长。                                                | `false`  |
| `DEBUG_ARTIFACT_RETENTION`  | 启用 debug artifacts 时保留的截图/HTML 对数。                                                             | `5`      |
